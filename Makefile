...
preview: ## Build docker image and start preview
	...
	$(DOCKER_BIN) run --rm -i -p 8081:80 -e "TEMPLATE_NAME=l7-light" $(APP_NAME):local

shell: ## Start shell into container with node
	$(DC_BIN) run $(DC_RUN_ARGS) app sh

docker.build.tag.push: ## vars: version
	docker build -t error-pages:latest .
	docker tag error-pages:latest kfirfer/error-pages:${version}
	docker push kfirfer/error-pages:${version}
